...
function Recipe(props) {
  ...
  async function fetchData() {
    ...
    const firstPartOfIngredients =
      typeof strIngredient !== "undefined" && strIngredient.length > 7
        ? strIngredient.slice(0, 8)
        : strIngredient;

    const secondPartOfIngredients =
      typeof strIngredient !== "undefined" && strIngredient.length > 7
        ? strIngredient.slice(9, strIngredient.length - 1)
        : filterIngredients;

    const firstPartOfMeasurements =
      typeof strMeasure !== "undefined" && strMeasure.length > 7
        ? strMeasure.slice(0, 8)
        : strMeasure;
