...
function Recipes() {
    ...
    return recipes.map((recipe, index) => {
      return (
        <div className="recipeGrp">
          <button
            onClick={(e) => handleCategoryListBtn(e)}
            value={recipe.strCategory}
          >
            {recipe.strCategory}
          </button>
          <button
            onClick={(e) => handleCuisineListBtn(e)}
  ...
  const renderCuisineList = () => {
    return cuisLists.map((cuisList, index) => {
      return (
        <CuisineCard name={cuisList.strMeal} image={cuisList.strMealThumb} />
      );
    });
  };
  ...
  return (
    <div className="recipesMain">
      ...
      <div className="renderDiv">
        <div>
          {renderCategoryList()} {renderCuisineList()}
        </div>
      </div>
    </div>
  );
